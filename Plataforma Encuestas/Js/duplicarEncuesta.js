...
async function renderizarFormularioEdicion(encuesta) {
  ...
  document.getElementById("btn-duplicar-encuesta").addEventListener("click", async () => {
    ...
    preguntaElements.forEach(preguntaDiv => {
      ...
      if (pregunta.tipo === 0) {
        const opcionesInputs = preguntaDiv.querySelectorAll('input[name="opcionTexto"]');
        ...
        opciones.push(opcionesPregunta); // Cada entrada es un array de opciones por pregunta
      } else {
        opciones.push([]);
      }
    });
    ...
    try {
      ...
      const res = await fetch(`https://localhost:7113/api/encuestas`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(encuesta)
      });
  
      const nuevaEncuesta = await res.json();
      const nuevaEncuestaId = nuevaEncuesta;
      console.log("Encuesta duplicada:", nuevaEncuesta);
  
      // Crear preguntas y opciones 
      for (let i = 0; i < preguntas.length; i++) {
        const pregunta = preguntas[i];
        pregunta.encuestaId = nuevaEncuestaId;
  
        const preguntaRes = await fetch(`https://localhost:7113/api/preguntas`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(pregunta)
        });
  
        const nuevaPregunta = await preguntaRes.json();
        const nuevaPreguntaId = nuevaPregunta;
  
        if (pregunta.tipo === 0) {
          for (const opcion of opciones[i]) {
            opcion.preguntaId = nuevaPreguntaId;
      ...
      alert("Hubo un error al duplicar la encuesta.");
    }
  });
